...
function lookUpProfile(name, prop) {
  ...
  for (let i = 0; i < contacts.length; i++) {
    if (name == contacts[i].firstName) {
      isFirstName = true;
      isProp = contacts[i].hasOwnProperty(prop);
      valor = i;
      break;
    }
  }

  if (isFirstName === true && isProp === true) {
    return contacts[valor][prop];
  } else if (isFirstName === false) {
    return "No such contact";
  } else if (isFirstName === true && isProp === false) {
